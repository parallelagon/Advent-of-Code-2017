# -*- coding: utf-8 -*-
"""
Created on Wed Dec 06 19:43:41 2017

@author: Zach
"""

data = '''vxjtwn vjnxtw sxibvv mmws wjvtxn icawnd rprh
...
fdata = data.split('\n')
ffdata = list(map((lambda x: x.split(' ')),fdata))


def repeatCount(teststring):
    stringvalue = 0
    for e in teststring:
        stringvalue += len(list(filter(lambda x : x == e, teststring)))
    if float(stringvalue)/len(teststring) == 1:
        return 1
    else:
        return 0

def numberValid(ffdata):
    return sum(list(map(repeatCount,ffdata)))

def isAnagram(word1,word2):
    intersection = filter(lambda x: x in list(word1),list(word2))
    if len(intersection) == len(word1) == len(word2):
        return 1
    else:
        return 0
    
def stringHasAnagram(teststring):
    teststring = teststring.split(' ')
    for wordindex in range(0,len(teststring)):
        for word in range(wordindex+1,len(teststring)):
            if isAnagram(teststring[wordindex],teststring[word]):
                return False
    return True
